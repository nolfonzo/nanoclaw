...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    // Handle errors gracefully with throttled logging to avoid spam
    this.bot.catch((err) => {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    try {
      ...
      if (text.length <= MAX_LENGTH) {
        ...
        if (chunks.length > TelegramChannel.MAX_MESSAGE_CHUNKS) {
          logger.warn({ jid, chunks: chunks.length }, 'Message too long, truncating to max chunks');
